...
async function main() {
  ...
  // 结算
  const reportPayouts = polyAbi.encodeFunctionData("reportPayouts", [condition1,0]);
  try {
    const tx = await deployer.sendTransaction({
      to: proxy.getAddress(),
      data: reportPayouts
    ...
    // @ts-ignore
    if (receipt.status === 1) {
      console.log("reportPayouts Transaction successful");
    } else {
      console.error("reportPayouts Transaction failed");
    }
  } catch (err) {
    console.error("Error during reportPayouts transaction:", err);
  }
