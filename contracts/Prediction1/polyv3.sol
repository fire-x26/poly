...
contract Option is ProxyTarget,Ownable{
    ...
    // prepare a new condition with multiple outcome slots
    function prepareCondition(string calldata conditionId, uint outcomeSlotCount) external onlyOwner{
        require(outcomeSlotCount <= 256, "too many outcome slots");
        require(outcomeSlotCount > 1, "there should be more than one outcome slot");
        require(payoutNumerators[conditionId].length == 0 && optionStatus[conditionId] == 0, "condition already prepared");
    ...
    // add KYC-approved addresses for a specific condition (addresses can be used to place bets)
    // function addKYCAddress(string calldata conditonId,address[] calldata  _addrs) external onlyOwner{
    //     require(optionStatus[conditonId] == 1, "condition not prepared");
    ...
    //     address _addr = KYCAddress[conditionId][position];
    //     require( optionStatus[conditionId] == 1,"not prepared");
    //     require(IERC20(casinoChips).balanceOf(_addr) >= amount, "Insufficient token balance");
    //     require(IERC20(casinoChips).transferFrom(_addr, address(this), amount), "Token transfer failed");
